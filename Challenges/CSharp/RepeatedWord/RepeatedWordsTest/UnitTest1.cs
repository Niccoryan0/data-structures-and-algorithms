using System;
using Xunit;
using static RepeatedWord.RepeatedWord;

namespace RepeatedWordsTest
{
    public class UnitTest1
    {
        ...
        public void CanFindRepeatedWord(string str, string expected)
        {
            string result = FindRepeat(str);

            Assert.Equal(expected, result);
        }       
        
        [Fact]
        public void CanReturnBlankForNoRepeats()
        {
            string result = FindRepeat("This sentence must contain no repeating words within it.");
