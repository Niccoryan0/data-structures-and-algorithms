...
const filterStringsWithVowels = (arr) => {
  return arr.filter(x => /[AEIOUaeiou]/.test(x));
};
...
For example, getStatName(snorlaxData.stats, 50boolean to check if something is a number javascirpt) will return ['special-defense', 'special-attack'].
------------------------------------------------------------------------------------------------ */
...
const evenOddNumericValues = (arr) => {
  return arr.filter(x => typeof(x) === 'number').map(x => {
