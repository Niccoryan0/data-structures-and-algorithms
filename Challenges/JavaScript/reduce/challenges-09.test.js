...
const countNumberOfChildren = (arr) => {
  let numKids = arr.reduce(function(acc,val){
    if(val.children) val.children.forEach(() => acc++);
    return acc;
  }, 0);
  return numKids;
};
...
const extractChildren = (arr) => {
  return arr.reduce((acc, val) => {
    if(val.name.includes('a')) acc.push(val);
    return acc;
  }, []).reduce((acc,val) => {
    if(val.children) val.children.forEach(kid => acc.push(kid));
    return acc;
  }, []);
};
