...
let biggerThanLuke = (arr) => {
  const lukeMass = arr.reduce((acc, val) => val.name === 'Luke Skywalker' ? acc = val.mass : acc,0);
...
Write a function named detect

    Expected: "Darth Vader - Pex Kylar"
    Received: undefined

      223 | describe('Testing challenge 3', () => {TicTacToeWin that accepts a two-dimensional array of strings. Each string is guaranteed to be either "X", "O" or an empty string. Your function should check to see if any row, column, or either diagonal direction has three matching "X" or "O" symbols (non-empty strings), three-in-a-line.
