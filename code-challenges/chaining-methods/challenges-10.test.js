...
const createServer = () => {
  ...
  const app = express();
  app.get('/hello', (req, res) => res.send('Hello from the back-end'));
  app.get('/aboutme', (req, res) => res.send('a short bio about you'));
  app.get('/favoritefoods', (req, res) => res.send(['food1', 'food2', 'food3']));
...
const count = (target, input) => {
  return input.reduce((acc, arr) => acc + arr.reduce((accu, val) => (val === target) ? accu+1 : accu, 0), 0);
};
...
const totalSum = (input) => {
  return input.reduce((acc, arr) => acc + arr.reduce((accu, val) => accu+val, 0), 0);
};
...
let findMaleAndFemale = (data) => {
  return data.reduce((acc, val) => {
    if(val.gender !== 'n/a' && val.name) acc.push(val.name);
    return acc;
  }, []).join(' and ');
...
let findShortest = (data) => {
  return data.sort((l,r) => l.height - r.height)[0].name;
};
